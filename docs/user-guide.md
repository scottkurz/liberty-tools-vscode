...
| --- | --- | --- |
| `maven.executable.path` | Maven commands executed by dev mode honour this setting from the [Maven for Java extension]((https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven), which takes precedence over all other options, including using the `mvnw` wrapper. | [Maven for Java extension](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven) |
| `maven.executable.preferMavenWrapper` | Maven commands executed by dev mode honour this setting. If the `maven.executable.path` setting is empty and this is set to `true`, dev mode tries to use `mvnw` if a Maven wrapper file can be found. Otherwise, it uses `mvn` from the PATH env variable. | [Maven for Java extension](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven) |
